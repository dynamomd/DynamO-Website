...
<p>
  ...
  running</i>. Although a lot of data can be measured from the
  configuration files, some properties (e.g., transport coefficients)
  must be measured using output plugins. There are a wide range of
  properties which can be measured by DynamO and these are listed
  below.
</p>
<p>
  ...
  default). Each output plugin has its own section in this file.
</p>
<p>
  DynamO automatically loads the <a href="#misc-plugin">Misc output
  plugin</a> as it samples a wide range of properties of the system
  which are computationally inexpensive to collect; however, all other
  output plugins must be enabled manually. How to enable a plugin is
...
<p>
  Standard plugins are plugins which collect data continuously
  throughout a simulation.  Most properties measured, such as
  temperature and mean free times, are collected through these types
  of plugins.
</p>
<h2>Misc</h2>
<p>
  The Misc plugin is the default plugin and is full of a range of
  properties which are relatively cheap to collect. Each of these
  properties has a corresponding sub-tag in the output file. The
  output tags of the Misc plugin and how they are collected are
